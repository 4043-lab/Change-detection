...
class GeM(nn.Module):
    ...
    def __init__(self, p=3, eps=1e-6, mode=None):
    ...
    def forward(self, x):
        if self.mode == 'H':
            return LF.gem_h(x, p=self.p, eps=self.eps)
        elif self.mode == 'W':
            return LF.gem_w(x, p=self.p, eps=self.eps)
        elif self.mode == 'G':
            return LF.gem_global(x, p=self.p, eps=self.eps)
...
class FR_block(nn.Module):
    def __init__(self, h, w):
        ...
        self.gem_w = GeM(mode='W')
        self.gem_h = GeM(mode='H')
    ...
    def forward(self, x):
        ...
        # f_cx1xw = self.avg_pool2(x)
        #
        # f1 = f_cxhx1.repeat(1, 1, 1, w)
        # f1_out = f1.permute(0, 2, 1, 3)
        #
        # f2 = f_cx1xw.repeat(1, 1, h, 1)
        # f2_out = f2.permute(0, 2, 1, 3)
        # out = torch.cat((f1_out, f2_out), 1)
...
class Spatial_self_optimization(nn.Module):
    def __init__(self):
        super(Spatial_self_optimization, self).__init__()
    ...
    def forward(self, input4, input3, input2, input1):
        x3 = self.edge_conv4(input4)
        t3, b3 = x3.chunk(2, dim=1)
        x3_ = t3 * input4 + b3
        x3 = self.up3(x3_) + input3
        x2 = self.edge_conv3(x3)
        t2, b2 = x2.chunk(2, dim=1)
        x2_ = t2 * x3 + b2
        x2 = self.up2(x2_) + input2
        x1 = self.edge_conv2(x2)
        t1, b1 = x1.chunk(2, dim=1)
        x1_ = t1 * x2 + b1
        x1 = self.up1(x1_) + input1
        x0 = self.edge_conv1(x1)
        t0, b0 = x0.chunk(2, dim=1)
        x0_ = t0 * x1 + b0
        xout = self.out_conv(x0_)
