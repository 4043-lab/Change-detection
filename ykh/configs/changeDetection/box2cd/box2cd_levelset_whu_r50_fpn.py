...
model = dict(
    ...
    bbox_head=dict(
        ...
        loss_sscr=dict(
            type='SscrLoss',
            loss_weight=1.0),
        loss_boxlevelset=dict(
            type='BoxLevelSetLoss',
            loss_weight=1.0
        )),
    train_cfg = dict(),
    test_cfg = dict(
...
data = dict(
    ...
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/WHU_CD_val.json',
...
log_level = 'INFO'
work_dir = './work_dirs/box2cd_levelset_whu_r50_v4_fpn'
load_from = None
resume_from = None
workflow = [('train', 1)]
