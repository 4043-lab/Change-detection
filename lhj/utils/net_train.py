...
def train_model(model, dataloaders, criterion, criterion_weight, optimizer, sc_plt, num_epochs=100, model_name='',
                model_info='', model_type='', multi_loss=False, dataset_name='', begin_epoch=0, best_f1=0.0):
    ...
    for epoch in range(begin_epoch, num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            # for sample in dataloaders[phase]:
                ...
                # labels_edge = (sample['label_edge'] > 0).type(torch.FloatTensor).cuda()

                # labels = (sample['label'] > 0).squeeze(1).type(torch.LongTensor).cuda()
                # edge label
                labels_edge_2 = (sample['label_edge'] > 0).type(torch.LongTensor).cuda()
                labels_edge_1 = torch.ones((labels_edge_2.shape[0], 1, labels_edge_2.shape[2], labels_edge_2.shape[3])).type(torch.LongTensor).cuda()
                labels_edge_1 = torch.sub(labels_edge_1, labels_edge_2)
                labels_edge = torch.cat((labels_edge_1, labels_edge_2), dim=1) # 构造一个2通道的edge label

                # focal label
                # labels_edge_2 = (sample['label_edge'] > 0).squeeze(1).type(torch.LongTensor).to(device)
                ...
                with torch.set_grad_enabled(phase == 'train'):  # train 计算梯度，val的时候不计算
                    ...
                    # loss_edge = criterion_weight[0] * criterion[0](F.softmax(image_out[0], dim=1), labels_edge.float())
                    # loss_seg = criterion_weight[1] * criterion[1](image_out[-1], labels)
                    # # print('edge:', loss_edge.item(), 'seg:', loss_seg.item())
                    # loss = loss_edge + loss_seg

                    loss_seg_1 = criterion_weight[-1] * criterion[-1](image_out[-1], labels)

                    loss_edge_3 = criterion_weight[0] * criterion[0](F.softmax(image_out[0], dim=1), labels_edge.float())  # mse_loss
                    loss_edge_4 = criterion_weight[1] * criterion[1](F.softmax(image_out[1], dim=1), labels_edge.float())

                    loss_edge = loss_edge_3 + loss_edge_4
                    # print(loss_edge)
                    loss_seg = loss_seg_1
                    loss = loss_edge + loss_seg
