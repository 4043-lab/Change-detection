...
class conv_e(nn.Module):
    def __init__(self, ch_in, ch_out, first_stride=2):
        super(conv_e, self).__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(ch_in, ch_out, kernel_size=3, stride=first_stride, padding=1, bias=False),
        ...
        # self.se1 = SEModule(ch_out, ch_out // 4)
        self.se2 = SEModule(ch_out, ch_out // 4)
        self.ReLU = nn.ReLU(inplace=True)
        self.shortcut =nn.Sequential(
            nn.Conv2d(ch_in, ch_out, kernel_size=1, stride=first_stride, bias=False),
            nn.BatchNorm2d(ch_out)
        )
...
class EGFDFN(nn.Module):
    def __init__(self, img_ch=3, output_ch=2, patch_size=256):
        ...
        # dual_encoder
        self.Conv1_e = conv_e(ch_in=img_ch, ch_out=16, first_stride=1)
        self.Conv2_e = conv_e(ch_in=16, ch_out=32, first_stride=2)
        self.Conv3_e = conv_e(ch_in=32, ch_out=64, first_stride=2)
        self.Conv4_e = conv_e(ch_in=64, ch_out=128, first_stride=2)
        self.Conv5_e = conv_e(ch_in=128, ch_out=256, first_stride=2)
