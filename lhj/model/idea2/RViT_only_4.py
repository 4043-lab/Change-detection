...
class Network(nn.Module):
    ...
    def pair_to_video(self, im1, im2, len):
        ...
        # return interped
        assert len >= 2
        video_dis = torch.abs(interped[1] - interped[0]).unsqueeze(0)
        for i in range(2, len):
            video_dis = torch.cat([video_dis, torch.abs(interped[i] - interped[i - 1]).unsqueeze(0)], dim=0)
        return video_dis
    ...
    def forward(self, t):
        ...
        video = self.pair_to_video(x1_e1, x1_e2, 5)
        # print(video.shape)
