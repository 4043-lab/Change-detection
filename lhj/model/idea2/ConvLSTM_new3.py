...
class ConvLSTMCell(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, kernel_size, bias):
        ...
        self.depth_conv = nn.Conv2d(in_channels=self.input_dim + self.hidden_dim,
        ...
        self.point_conv = nn.Conv2d(in_channels=self.input_dim + self.hidden_dim,
                                    out_channels=4 * self.hidden_dim,
                                    kernel_size=1,
                                    padding=0,
                                    bias=self.bias)
    ...
    def forward(self, input_tensor, cur_state):
        ...
        combined_conv = self.depth_conv(combined)
        combined_conv = self.point_conv(combined_conv)
        cc_i, cc_f, cc_o, cc_g = torch.split(combined_conv, self.hidden_dim, dim=1)
        # cc_i = combined_conv
        # cc_f = combined_conv
        # cc_o = combined_conv
        # cc_g = combined_conv
        # cc_i = self.conv_i(combined_conv)
        # cc_f = self.conv_f(combined_conv)
        # cc_o = self.conv_o(combined_conv)
        # cc_g = self.conv_g(combined_conv)
        print(cc_i.shape)
...
class ConvLSTM(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state=None, reverse=False):
        ...
        for layer_idx in range(self.num_layers):
            ...
            # 执行conv_unit
            if not reverse:
                range_t = range(seq_len)
            else:
                range_t = range(seq_len-1,-1,-1)
            for t in range_t:
...
if __name__ == '__main__':
    net = ConvLSTM(input_dim=512, hidden_dim=512, kernel_size=(3,3), num_layers=2, batch_first=False, bias=True, return_all_layers=False).cuda()
    ...
    getModelSize(net)
    x = torch.randn([5, 8, 512, 256, 256]).cuda()
    y = net(x)
    print(y.shape)