...
class Bottleneck(nn.Module):
    def __init__(self, in_ch, out_ch, k, downsample=True):
        ...
        self.conv3_1 = nn.Sequential(
            ...
            CondConv(out_ch // self.expansion, out_ch // self.expansion, kernel_size=3, stride=self.down_stride, padding=1, bias=False, K=k),
            nn.BatchNorm2d(out_ch // self.expansion),
            nn.ReLU(inplace=True)
        )
...
class decoder(nn.Module):
    def __init__(self, in_ch, out_ch, k):
        ...
        self.conv = Bottleneck(in_ch, out_ch, k=k, downsample=False)
...
class Network(nn.Module):
    def __init__(self, k):
        ...
        self.res1 = nn.Sequential(
            Bottleneck(64, 128, k=k),
            Bottleneck(128, 128, k=k, downsample=False),
            Bottleneck(128, 128, k=k, downsample=False)
        )
        self.res2 = nn.Sequential(
            Bottleneck(128, 256, k=k),
            Bottleneck(256, 256, k=k, downsample=False),
            Bottleneck(256, 256, k=k, downsample=False),
            Bottleneck(256, 256, k=k, downsample=False)
        )
        self.res3 = nn.Sequential(
            Bottleneck(256, 512, k=k),
            Bottleneck(512, 512, k=k, downsample=False),
            Bottleneck(512, 512, k=k, downsample=False),
            Bottleneck(512, 512, k=k, downsample=False),
            Bottleneck(512, 512, k=k, downsample=False),
            Bottleneck(512, 512, k=k, downsample=False)
        ...
        self.res4 = nn.Sequential(
            Bottleneck(512, 512, k=k),
            Bottleneck(512, 512, k=k, downsample=False),
            Bottleneck(512, 512, k=k, downsample=False),
        )

        self.decoder1 = decoder(1024, 512, k=k)
        self.decoder2 = decoder(512, 256, k=k)
        self.decoder3 = decoder(256, 128, k=k)
        self.decoder4 = decoder(128, 128, k=k)

        self.head = nn.Conv2d(128, 2, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, t1, t2):
        t1 = F.interpolate(t1, scale_factor=2, mode='bilinear', align_corners=True, recompute_scale_factor=True)
        t2 = F.interpolate(t2, scale_factor=2, mode='bilinear', align_corners=True, recompute_scale_factor=True)
        ...
        out1 = F.interpolate(out1, scale_factor=0.5, mode='bilinear', align_corners=True, recompute_scale_factor=True)
        out2 = F.interpolate(out2, scale_factor=0.5, mode='bilinear', align_corners=True, recompute_scale_factor=True)
        out3 = F.interpolate(out3, scale_factor=0.5, mode='bilinear', align_corners=True, recompute_scale_factor=True)
        out4 = F.interpolate(out4, scale_factor=0.5, mode='bilinear', align_corners=True, recompute_scale_factor=True)
        out = F.interpolate(out, scale_factor=0.5, mode='bilinear', align_corners=True, recompute_scale_factor=True)

        return x1_e1, x2_e1, x3_e1, x4_e1, x5_e1, x1_e2, x2_e2, x3_e2, x4_e2, x5_e2, out1, out2, out3, out4, out
