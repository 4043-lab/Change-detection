...
class Bottleneck(nn.Module):
    def __init__(self, in_ch, out_ch, downsample=True):
        ...
        self.conv3_1 = nn.Sequential(
            ...
            CondConv(out_ch // self.expansion, out_ch // self.expansion, kernel_size=3, stride=self.down_stride, padding=1, bias=False, K=8),
            nn.BatchNorm2d(out_ch // self.expansion),
            nn.ReLU(inplace=True)
        )
...
class Network(nn.Module):
    ...
    def forward(self, t):
        # resize1 = transforms.Resize(size=512, interpolation=InterpolationMode.BILINEAR)
        # resize2 = transforms.Resize(size=256, interpolation=InterpolationMode.BILINEAR)
