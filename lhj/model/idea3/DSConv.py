...
class DSConv_pro(nn.Module):
    def __init__(
        ...
        self.morph = morph  # 0 x方向    1 y方向
        self.if_offset = if_offset
        self.device = torch.device(device)
        self.to(device)

        # self.bn = nn.BatchNorm2d(2 * kernel_size)
        self.gn_offset = nn.GroupNorm(kernel_size, 2 * kernel_size)  # 将channel方向分group，然后每个group内做归一化. num_groups：需要划分为的groups num_features：输入通道数
    ...
    def forward(self, input: torch.Tensor):
        ...
        offset = self.offset_conv(input)  # offset：卷积核大小*2个通道
        # print(offset.shape)
...
def get_coordinate_map_2D(
    ...
    if morph == 0:
        ...
        x_new_ = x_center_ + x_grid_  # 每个通道表示卷积核中的每个位置在图像中每个(x，y)处的初始偏移量

        y_new_ = einops.repeat(y_new_, "k w h -> b k w h", b=batch_size)
        x_new_ = einops.repeat(x_new_, "k w h -> b k w h", b=batch_size)

        y_offset_ = einops.rearrange(y_offset_, "b k w h -> k b w h")  # 某一位置相对于其上一个位置的偏移量
        ...
        # 采用迭代的方法从中间向两边逐个计算y方向的偏移量，保持卷积核的连续性
...
if __name__ == '__main__':
    dsconv_x = DSConv_pro(64, 64, 9, 1, 0, True).cuda()
    x = torch.ones([1, 64, 256, 256]).cuda()
    y = dsconv_x(x)
    # print(y.shape)