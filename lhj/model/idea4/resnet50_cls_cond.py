...
class Bottleneck(nn.Module):
    def __init__(self, in_ch, out_ch, downsample=True):
        ...
        self.conv3_1 = nn.Sequential(
            ...
            CondConv(out_ch // self.expansion, out_ch // self.expansion, kernel_size=3, stride=1, padding=1, bias=False, K=4),
            nn.BatchNorm2d(out_ch // self.expansion),
            nn.ReLU(inplace=True)
        )
...
class decoder(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        # self.conv = Bottleneck(in_ch + out_ch, out_ch, downsample=False)
        self.conv = nn.Sequential(
            nn.Conv2d(in_ch + out_ch, out_ch, 1, 1, 0, bias=False),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )
        self.maxpooling = nn.MaxPool2d(2)
...
class Network(nn.Module):
    def __init__(self):
        ...
        self.decoder2 = decoder(256, 512)
        self.decoder3 = decoder(512, 1024)
        self.decoder4 = decoder(1024, 1024)
    ...
    def forward(self, t):
        ...
        # return out
        # return x1_e1, x2_e1, x3_e1, x4_e1, x5_e1, x1_e2, x2_e2, x3_e2, x4_e2, x5_e2, out
