...
class decoder_aux(nn.Module):
    def __init__(self, in_ch, out_ch, upsample=True):
        super(decoder_aux, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, 1, 1, 0, bias=False),
            nn.BatchNorm2d(out_ch),
            nn.ReLU()
...
class Network(nn.Module):
    def __init__(self):
        ...
        self.decoder1_aux = decoder_aux(2048+1024, 1024, upsample=False)
        self.decoder2_aux = decoder_aux(1024+512, 512, upsample=True)
        self.decoder3_aux = decoder_aux(512+256, 256, upsample=True)
    ...
    def forward_origin(self, t):
        ...
        x4_d1 = x4_e1
        x3_d1 = self.decoder1_aux(x4_d1, x3_e1)
        x2_d1 = self.decoder2_aux(x3_d1, x2_e1)
        x1_d1 = self.decoder3_aux(x2_d1, x1_e1)
        x4_d2 = x4_e2
        x3_d2 = self.decoder1_aux(x4_d2, x3_e2)
        x2_d2 = self.decoder2_aux(x3_d2, x2_e2)
        x1_d2 = self.decoder3_aux(x2_d2, x1_e2)
