...
class EGFDFN(nn.Module):
    def __init__(self, img_ch=3, output_ch=2, patch_size=256):
        ...
        model_load = "result/T1_add_T2/checkpoints/best_model_epoch068_f_score0.8344_iou0.7159.pth"
        self.T1_add_T2 = torch.load(model_load)
        self.freeze(self.T1_add_T2)
    ...
    def encoder(self, x):
        # x0 = torch.cat([x[0].unsqueeze(0), x[0].unsqueeze(0)], dim=0)
        # x1 = torch.cat([x[1].unsqueeze(0), x[1].unsqueeze(0)], dim=0)
        x0 = x[0].unsqueeze(0)
        x1 = x[1].unsqueeze(0)

        x0 = self.T1_add_T2(x0)
        x1 = self.T1_add_T2(x1)

        x1_e1 = self.Conv1_e(x0)
        x2_e1 = self.Conv2_e(x1_e1)
        x3_e1 = self.Conv3_e(x2_e1)
        x4_e1 = self.Conv4_e(x3_e1)
        x5_e1 = self.Conv5_e(x4_e1)
        x1_e2 = self.Conv1_e(x1)
