...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, downsample=True):
        ...
        self.conv1_1 = nn.Sequential(
            ...
            CondConv(in_planes=in_ch, out_planes=out_ch // self.expansion, kernel_size=1, stride=1, padding=0, bias=False, K=8),
        ...
        self.conv3_1 = nn.Sequential(
            ...
            CondConv(in_planes=out_ch // self.expansion, out_planes=out_ch // self.expansion, kernel_size=3, stride=self.down_stride, padding=1, bias=False, K=8),
            nn.BatchNorm2d(out_ch // self.expansion),
            nn.ReLU(inplace=True)
        )
        self.conv1_2 = nn.Sequential(
            # nn.Conv2d(out_ch // self.expansion, out_ch, kernel_size=1, stride=1, padding=0, bias=False),
            CondConv(in_planes=out_ch // self.expansion, out_planes=out_ch, kernel_size=1, stride=1, padding=0, bias=False, K=8),
...
class Resnet_bottle(nn.Module):
    def __init__(self):
        ...
        self.stem = nn.Sequential(
            ...
            # CondConv(in_planes=3, out_planes=64, kernel_size=3, stride=1, padding=1, bias=False, K=8),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False),
            # CondConv(in_planes=64, out_planes=64, kernel_size=3, stride=1, padding=1, bias=False, K=8),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
        )
